...
export class TunnelManager {
  ...
  installPlan(kind: TunnelKind): Record<string, unknown> {
    return kind === "cloudflared"
      ? {
          ...
          commands: process.platform === "win32"
            ...
            : process.platform === "darwin"
              ...
              : ["Install the official cloudflared package for your distribution"]
        }
      : { source: "https://github.com/fatedier/frp/releases/latest", commands: ["Download the matching official frp archive, verify its checksum, and place frpc on PATH"] };
  }
  async install(kind: TunnelKind, confirmed: boolean): Promise<TunnelInstallResult> {
    ...
    const osName = process.platform === "win32" ? "windows" : process.platform === "darwin" ? "darwin" : "linux"; const archName = process.arch === "arm64" ? "arm64" : "amd64";
    const cloudflaredAsset = `cloudflared-${osName}-${archName}${process.platform === "win32" ? ".exe" : process.platform === "darwin" ? ".tgz" : ""}`;
    const asset = release.assets?.find((item) => kind === "cloudflared" ? item.name === cloudflaredAsset : item.name.includes(`${osName}_${archName}`) && item.name.endsWith(process.platform === "win32" ? ".zip" : ".tar.gz"));
    ...
    const archiveAsset = asset.name.endsWith(".zip") || asset.name.endsWith(".tar.gz") || asset.name.endsWith(".tgz");
    if (!archiveAsset) await writeFile(destination, bytes, { mode: 0o755 });
    else {
      ...
      if (unpack.status !== 0) { await rm(temp, { recursive: true, force: true }); throw new AppError("EXTRACT_FAILED", `failed to extract ${kind} release archive`, 500); }
      const source = kind === "cloudflared"
        ? path.join(extract, "cloudflared")
        : path.join(extract, `frp_${release.tag_name?.replace(/^v/, "")}_${osName}_${archName}`, `${kind}${process.platform === "win32" ? ".exe" : ""}`);
      await copyFile(source, destination); if (process.platform !== "win32") await chmod(destination, 0o755); await rm(temp, { recursive: true, force: true });
    }
